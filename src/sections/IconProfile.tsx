        ...
        <div className="relative max-w-[1500px] mx-auto px-[6vw] md:px-[5vw] py-[10vh]">
          ...
          >
            ...
            <div className="mt-8 flex flex-wrap items-baseline gap-x-10 gap-y-3">
              <span className="t-label">{icon.discipline}</span>
          ...
          <div className="grid md:grid-cols-[minmax(0,1fr)_minmax(0,1.25fr)] gap-[6vw]">
            ...
            >
              ...
              <p className="t-lead mt-7">{icon.line}</p>
              ...
              {PHOTO_CREDITS[icon.id] ? (
                <p className="mt-6 text-[0.58rem] tracking-[0.18em] uppercase text-smoke/60 leading-relaxed">
                  Photograph: {PHOTO_CREDITS[icon.id].artist}
                  <br />
                ...
                <p className="mt-6 text-[0.58rem] tracking-[0.2em] uppercase text-smoke/60 leading-relaxed">
                  Generative illustration — not a photograph
                </p>
              )}
            </motion.div>
            ...
            >
              ...
              <section>
                <SectionTitle>Cultural impact</SectionTitle>
                <p className="t-lead">{icon.impact}</p>
              </section>
              ...
              <section>
                <SectionTitle>Major works</SectionTitle>
                <div className="grid sm:grid-cols-2 gap-x-8">
                  {icon.works.map((w) => (
                    <div key={w.title} className="hair-t py-5">
                      <div className="flex items-baseline justify-between gap-4 mb-2">
                        <h4 className="font-display text-xl leading-tight">{w.title}</h4>
                        <span className="t-eyebrow tabular-nums shrink-0">{w.year}</span>
                      </div>
                      <p className="text-[0.82rem] text-smoke leading-relaxed font-light">{w.note}</p>
                    </div>
                  ))}
                </div>
              </section>
              ...
              <section>
                ...
                <ol className="relative pl-7">
                  ...
                  {icon.timeline.map((t) => (
                    <li key={t.year + t.event} className="relative pb-7 last:pb-0">
                      <span
                        className="absolute -left-7 top-[0.45rem] w-[7px] h-[7px] rounded-full"
                        style={{ background: icon.palette[0] }}
                      />
                      <p className="t-eyebrow tabular-nums mb-1">{t.year}</p>
                      <p className="text-[0.92rem] text-bone/85 font-light leading-relaxed">{t.event}</p>
                    </li>
                  ))}
                </ol>
              </section>
